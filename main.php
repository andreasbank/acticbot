...
class ActicBot {
  ...
  private function parse_trainings($data) {
    $pattern = '/visible-xs.*getBookingDetails\((?<booking_id>\d+), (?<activity_id>\d+), (?<center_id>\d+), \'(?<date>.*)\',\s*\$\(this\)\);/U';
    $result = preg_match_all($pattern, $data, $matches);
    ...
    if ($result != false && $result > 0) {
      for ($i = 0; $i < $result; $i++) {
        $trainings[] = array(
            'booking_id' => $matches['booking_id'][$i],
            'activity_id' => $matches['activity_id'][$i],
            'center_id' => $matches['center_id'][$i],
            'date' => $matches['date'][$i],
            );
  ...
  public function check_booking($day, $center_id, $pass, $time, $instructor) {
...
var_dump($centers);
*/
$booked = $acticbot->check_booking('friday', '17:00 - 17:55',
    'Functional Toning by Actic', 24, 'Michail Triantafyllidis');
var_dump($booked);
